import { useEffect, useMemo, useState } from 'react'
import { useParams, Navigate, Link } from 'react-router-dom'
import {
  ArrowLeft, CheckCircle2, ChevronLeft, ChevronRight,
  Download, Moon, Sun, BookOpen, PlayCircle,
} from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'
import {
  getCourse, isEnrolled, getProgress,
  markLessonComplete, flattenLessons,
} from '../lib/data'
import Sidebar from '../components/Sidebar'
import VideoPlayer from '../components/VideoPlayer'

export default function CoursePlayer() {
  const { courseId } = useParams()
  const { user }     = useAuth()

  const [course,       setCourse]       = useState(null)
  const [allowed,      setAllowed]      = useState(null)
  const [completedIds, setCompletedIds] = useState([])
  const [activeLesson, setActiveLesson] = useState(null)
  const [sidebarOpen,  setSidebarOpen]  = useState(false)
  const [readMode,     setReadMode]     = useState('light')

  const lessons = useMemo(() => course ? flattenLessons(course) : [], [course])

  useEffect(() => {
    if (!user) return
    isEnrolled(user.uid, courseId).then(setAllowed)
    getCourse(courseId).then(setCourse)
    getProgress(user.uid, courseId).then(p => setCompletedIds(p.completedLessonIds))
  }, [user, courseId])

  useEffect(() => {
    if (lessons.length && !activeLesson) setActiveLesson(lessons[0])
  }, [lessons])

  if (allowed === false) return <Navigate to={`/courses/${courseId}`} replace />
  if (!course || allowed === null) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center">
      <div className="h-6 w-6 rounded-full border-2 border-gold border-t-transparent animate-spin" />
    </div>
  )

  async function toggleComplete() {
    const done = !completedIds.includes(activeLesson.id)
    await markLessonComplete(user.uid, courseId, activeLesson.id, done)
    setCompletedIds(ids =>
      done ? [...ids, activeLesson.id] : ids.filter(i => i !== activeLesson.id)
    )
  }

  const lessonIdx  = lessons.findIndex(l => l.id === activeLesson?.id)
  const hasPrev    = lessonIdx > 0
  const hasNext    = lessonIdx < lessons.length - 1
  const isDone     = completedIds.includes(activeLesson?.id)
  const isDarkText = activeLesson?.type === 'text' && readMode === 'dark'

  function goTo(idx) {
    if (idx >= 0 && idx < lessons.length) setActiveLesson(lessons[idx])
    setSidebarOpen(false)
  }

  return (
    <div className="min-h-screen flex">
      <Sidebar
        course={course}
        activeLessonId={activeLesson?.id}
        completedIds={completedIds}
        onSelect={l => { setActiveLesson(l); setSidebarOpen(false) }}
        open={sidebarOpen}
        onToggle={() => setSidebarOpen(o => !o)}
      />

      <main className="flex-1 min-w-0 overflow-auto">
        <div
          className={`min-h-screen transition-colors duration-200
            ${isDarkText ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-900'}`}
        >
          <div className="max-w-3xl mx-auto px-5 sm:px-8 py-8 lg:py-12">

            {/* Back + breadcrumb */}
            <div className="flex items-center gap-3 mb-8 mt-8 lg:mt-0">
              <Link
                to="/dashboard"
                className={`inline-flex items-center gap-1.5 text-xs font-medium transition
                  ${isDarkText ? 'text-slate-400 hover:text-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
              >
                <ArrowLeft className="h-3.5 w-3.5" />
                My library
              </Link>
              {activeLesson && (
                <>
                  <span className={`text-xs ${isDarkText ? 'text-slate-700' : 'text-slate-300'}`}>/</span>
                  <span className={`text-xs ${isDarkText ? 'text-slate-500' : 'text-slate-400'}`}>
                    Module {activeLesson.moduleIndex}: {activeLesson.moduleTitle}
                  </span>
                </>
              )}
            </div>

            {/* Lesson header */}
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-mono text-xs font-semibold text-gold bg-gold/10 px-2 py-0.5 rounded">
                  {activeLesson?.lessonNumber}
                </span>
                <span className={`text-xs font-medium flex items-center gap-1
                  ${isDarkText ? 'text-slate-500' : 'text-slate-400'}`}>
                  {activeLesson?.type === 'video'
                    ? <><PlayCircle className="h-3 w-3" /> Video</>
                    : <><BookOpen className="h-3 w-3" /> Reading</>}
                </span>
                {activeLesson?.durationMin && (
                  <span className={`text-xs font-mono ${isDarkText ? 'text-slate-600' : 'text-slate-400'}`}>
                    · {activeLesson.durationMin} min
                  </span>
                )}
              </div>
              <h1 className={`text-2xl sm:text-3xl font-bold leading-tight tracking-tight
                ${isDarkText ? 'text-white' : 'text-slate-900'}`}>
                {activeLesson?.title}
              </h1>
            </div>

            {/* Content */}
            <div className="mt-6">
              {activeLesson?.type === 'video' ? (
                <VideoPlayer youtubeId={activeLesson.youtubeId} onEnded={() => {}} />
              ) : (
                <div className="flex justify-end mb-5">
                  <button
                    onClick={() => setReadMode(m => m === 'light' ? 'dark' : 'light')}
                    className={`inline-flex items-center gap-2 text-xs font-medium rounded-full px-3.5 py-1.5 border transition
                      ${isDarkText
                        ? 'border-white/10 text-slate-400 hover:text-slate-200'
                        : 'border-slate-200 text-slate-500 hover:text-slate-700'}`}
                  >
                    {readMode === 'light'
                      ? <><Moon className="h-3.5 w-3.5" /> Dark mode</>
                      : <><Sun  className="h-3.5 w-3.5" /> Light mode</>}
                  </button>
                </div>
              )}

              {/* Text content */}
              {activeLesson?.type === 'text' && (
                <article
                  className={`text-base sm:text-[1.05rem] leading-[1.85] whitespace-pre-line
                    ${isDarkText ? 'text-slate-300' : 'text-slate-700'}`}
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
                  {activeLesson.content}
                </article>
              )}

              {/* Downloads */}
              {!!activeLesson?.resources?.length && (
                <div className={`mt-10 pt-6 border-t ${isDarkText ? 'border-white/10' : 'border-slate-100'}`}>
                  <p className={`text-xs font-mono uppercase tracking-wider mb-3
                    ${isDarkText ? 'text-slate-500' : 'text-slate-400'}`}>
                    Downloads
                  </p>
                  <ul className="space-y-2">
                    {activeLesson.resources.map((r, i) => (
                      <li key={i}>
                        <a
                          href={r.url}
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-2 text-sm text-gold hover:underline font-medium"
                        >
                          <Download className="h-4 w-4" />
                          {r.label}
                        </a>
                      </li>
                    ))}
                  </ul>
                </div>
              )}
            </div>

            {/* Bottom nav: prev / mark complete / next */}
            <div className={`mt-12 pt-6 border-t flex items-center justify-between gap-3
              ${isDarkText ? 'border-white/10' : 'border-slate-100'}`}>

              {/* Previous */}
              <button
                onClick={() => goTo(lessonIdx - 1)}
                disabled={!hasPrev}
                className={`inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium transition
                  ${hasPrev
                    ? isDarkText
                      ? 'text-slate-300 hover:bg-white/5 border border-white/10'
                      : 'text-slate-600 hover:bg-slate-50 border border-slate-200'
                    : 'opacity-30 cursor-not-allowed border border-transparent text-slate-400'}`}
              >
                <ChevronLeft className="h-4 w-4" />
                Previous
              </button>

              {/* Mark complete */}
              <button
                onClick={toggleComplete}
                className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition
                  ${isDone
                    ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                    : 'bg-slate-900 text-white hover:bg-slate-700'}`}
              >
                {isDone && <CheckCircle2 className="h-4 w-4" />}
                {isDone ? 'Completed' : 'Mark as complete'}
              </button>

              {/* Next */}
              <button
                onClick={() => goTo(lessonIdx + 1)}
                disabled={!hasNext}
                className={`inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-semibold transition
                  ${hasNext
                    ? 'bg-gold text-slate-900 hover:bg-goldDeep'
                    : 'opacity-30 cursor-not-allowed bg-slate-100 text-slate-400'}`}
              >
                Next
                <ChevronRight className="h-4 w-4" />
              </button>
            </div>

            {/* Lesson counter */}
            <p className={`text-center text-xs font-mono mt-4
              ${isDarkText ? 'text-slate-700' : 'text-slate-300'}`}>
              {lessonIdx + 1} of {lessons.length} lessons
            </p>
          </div>
        </div>
      </main>
    </div>
  )
}